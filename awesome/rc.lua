...
mysettingsmenu = {
    { "all settings", "xfce4-settings-manager" },
    { "display settings", "xfce4-display-settings --minimal" },
...
awful.screen.connect_for_each_screen(function(s)
    ...
    s.mytaglist = awful.widget.taglist(s, awful.widget.taglist.filter.all, taglist_buttons, {}, function(w, buttons, label, data, objects)
        ...
        for i = 1, #objects do
            ...
            if o.selected then
                p.bgb.shape = function(cr)
                    gears.shape.transform(gears.shape.powerline) : translate(p.tbm.left - 7 - 7, -1) (cr, 32, 20)
                end
            elseif i > 1 and objects[i-1].selected then
                p.bgb:set_bg(beautiful.taglist_bg_focus)
                p.bgb.shape = function(cr)
                    gears.shape.transform(gears.shape.powerline) : translate(p.tbm.left - 7 - 28, -1) (cr, 32, 20)
                end
            elseif  i < #objects and objects[i+1].selected then
                p.bgb:set_bg(beautiful.taglist_bg_focus)
                p.bgb.shape = function(cr)
                    gears.shape.transform(gears.shape.powerline) : translate(p.tbm.left - 7 + 14, -1) (cr, 32, 20)
                end
            end
        end
    end)
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
            ...
            wibox.container.background(wibox.container.margin(wibox.widget.systray(), 5, 5), beautiful.bg_systray),
            separators.arrow_left(beautiful.bg_systray, beautiful.bg_clock),
            wibox.container.background(wibox.container.margin(mytextclock, 5, 5), beautiful.bg_clock),
            separators.arrow_left(beautiful.bg_clock, beautiful.bg_layout),
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey            }, "p", function () awful.spawn("xfce4-display-settings --minimal") end,
              {description = "display settings", group = "launcher"}),

    awful.key({ modkey, "Control" }, "l", function () awful.spawn("xflock4") end,
